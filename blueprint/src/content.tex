...
(E72)
Euler, Leonhard, Variae observations circa series infinitas, Commentarii academiae scientiarum
...
\begin{proof}
\leanok
    We use Mathlib lemma \verb|log_add_one_le_harmonic| which proves the inequality for $x=n+1$. The conditions are equivalent to $n=\lfloor x\rfloor$. 
\end{proof}
...
\lean{H_P4_1}
\leanok
    Given the infinite set $S_x$ of $\{1\}\cup$ all numbers $m$ that have only prime factors $p\le x$,
    $$\sum_{k=1}^n\frac1k \le \sum_{m\in S_x}\frac{1}{m}. $$
\end{lemma}
The argument in Aigner \& Ziegler, 6th ed. had the set not containing the inverse of~$1$ which
